...
.. _program_listing_file_src_InputData_NucData_NuMagSANSlib_NucDataExplorer.h:

Program Listing for File NuMagSANSlib_NucDataExplorer.h
=======================================================

|exhale_lsh| :ref:`Return to documentation for file <file_src_InputData_NucData_NuMagSANSlib_NucDataExplorer.h>` (``src/InputData/NucData/NuMagSANSlib_NucDataExplorer.h``)
...
.. code-block:: cpp
   ...
   bool check_number_of_elements_in_folders_NucData(NucDataProperties* NucDataProp) {
       ...
       for (int k = 0; k < NucDataProp->Number_Of_SubFolders; k++) {
           current_path =
               NucDataProp->GlobalFolderPath + "/" + NucDataProp->SubFolderNames_Nom + "_" + std::to_string(k + 1);
   ...
   bool check_element_names_NucData(NucDataProperties* NucDataProp) {
       ...
       for (int k = 0; k < NucDataProp->Number_Of_SubFolders; k++) {
           ...
           CheckStrings_ElementNames(ElementNames, NucDataProp->Number_Of_Files_In_SubFolder, &ElementNames_CheckFlag,
   ...
   bool check_Subfolders_NucData(NucDataProperties* NucDataProp) {
       ...
       CheckStrings_FolderNames(SubFolderNames, NucDataProp->Number_Of_SubFolders, &FolderNames_CheckFlag,
                                &NucDataProp->SubFolderNames_Nom);
   ...
   bool check_Subfolder_FileNames_NucData(NucDataProperties* NucDataProp) {
       ...
       if (Elements_CheckFlag) {
           ...
           if (Element_Names_CheckFlag) {
               LogSystem::write("We found the Nom: " + NucDataProp->SubFolder_FileNames_Nom +
                                ", and the file Type: " + NucDataProp->SubFolder_FileNames_Type);
               LogSystem::write("");
               return true;
           } else {
               return false;
   ...
   bool check_FileDimensions_NucData(NucDataProperties* NucDataProp, bool FastLoad) {
       ...
       for (int i = 0; i < NucDataProp->Number_Of_SubFolders; i++) {
           for (int j = 0; j < NucDataProp->Number_Of_Files_In_SubFolder; j++) {
               if (!FastLoad || j == 0) {
                   filename = NucDataProp->GlobalFolderPath + "/" + NucDataProp->SubFolderNames_Nom + "_" +
                              std::to_string(i + 1) + "/" + NucDataProp->SubFolder_FileNames_Nom + "_" +
                              std::to_string(j + 1) + "." + NucDataProp->SubFolder_FileNames_Type;
                   // NumberOfNonZeroMagneticMomentsInFile(&NucDataProp->NumberOfNonZeroMoments[i][j],
                   // &NucDataProp->NumberOfElements[i][j], filename);
   ...
   bool NucData_Observer(std::string Local_NucDataPath, NucDataProperties* NucDataProp, InputFileData* InputData) {
       ...
       if (InputData->NucData_ReplicationImport_flag) {
           ReplicationImport_CheckFlag =
               ApplyReplicationImport_NucData(NucDataProp, InputData->NucData_NumberOfReplications);
       }
   
       CountAtomNumbers_NucData(NucDataProp);
   
       for (int i = 0; i < NucDataProp->Number_Of_Files_In_SubFolder; i++) {
           LogSystem::write("total number of atoms in the data set " + std::to_string(i + 1) + ": " +
                            std::to_string(NucDataProp->TotalAtomNumber[i]));
       }
