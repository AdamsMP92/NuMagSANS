...
.. _program_listing_file_src_gpu_kernels_AtomisticNucSANS_StructRotKernel.h:

Program Listing for File AtomisticNucSANS_StructRotKernel.h
===========================================================

|exhale_lsh| :ref:`Return to documentation for file <file_src_gpu_kernels_AtomisticNucSANS_StructRotKernel.h>` (``src/gpu_kernels/AtomisticNucSANS_StructRotKernel.h``)
...
.. code-block:: cpp
   ...
   __global__ void Atomistic_NucSANS_Kernel_StructRot(NuclearData NucData, StructureData StructData, RotationData RotData,
       ...
       if (i < L) {
           for (int k = 0; k < K; k++) {
               ...
               for (int l = 0; l < N_act; l++) {
                   ...
                   Y = NucData.RotMat[1] * (xr + StructData.x[k]) + NucData.RotMat[4] * (yr + StructData.y[k]) +
                       NucData.RotMat[7] * (zr + StructData.z[k]);
                   Z = NucData.RotMat[2] * (xr + StructData.x[k]) + NucData.RotMat[5] * (yr + StructData.y[k]) +
