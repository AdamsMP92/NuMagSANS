...
.. _program_listing_file_src_gpu_kernels_AtomisticMagSANS_StructRotKernel.h:

Program Listing for File AtomisticMagSANS_StructRotKernel.h
===========================================================

|exhale_lsh| :ref:`Return to documentation for file <file_src_gpu_kernels_AtomisticMagSANS_StructRotKernel.h>` (``src/gpu_kernels/AtomisticMagSANS_StructRotKernel.h``)
...
.. code-block:: cpp
   ...
   // Computes purely magnetic atomistic SANS for a structured orientation
   // ensemble.
   // Active data layers:
   ...
   //   - NuclearData
   // Use case:
   //   Magnetic scattering from an assembly with explicit object positions and
   //   object-wise local orientations.
   //
   // Placeholder for:
   // Atomistic_MagSANS_Kernel_StructRot
   
   __global__ void Atomistic_MagSANS_Kernel_StructRot(MagnetizationData MagData, StructureData StructData,
                                                      RotationData RotData, ScatteringData SANSData) {
       ...
       if (i < L) {
           for (int k = 0; k < (*MagData.K); k++) {
               ...
               for (int l = 0; l < N_act; l++) {
                   ...
                   Y = MagData.RotMat[1] * (xr + StructData.x[k]) + MagData.RotMat[4] * (yr + StructData.y[k]) +
                       MagData.RotMat[7] * (zr + StructData.z[k]);
                   Z = MagData.RotMat[2] * (xr + StructData.x[k]) + MagData.RotMat[5] * (yr + StructData.y[k]) +
