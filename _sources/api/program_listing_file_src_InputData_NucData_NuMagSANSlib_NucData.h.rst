...
.. _program_listing_file_src_InputData_NucData_NuMagSANSlib_NucData.h:

Program Listing for File NuMagSANSlib_NucData.h
===============================================

|exhale_lsh| :ref:`Return to documentation for file <file_src_InputData_NucData_NuMagSANSlib_NucData.h>` (``src/InputData/NucData/NuMagSANSlib_NucData.h``)
...
.. code-block:: cpp
   ...
   void allocate_NuclearDataRAM(NuclearData* NucData, NucDataProperties* NucDataProp, int NucData_File_Index) {
       ...
       if (!NucData->x || !NucData->y || !NucData->z || !NucData->Nuc || !NucData->N_avg || !NucData->K ||
           !NucData->TotalAtomNumber || !NucData->RotMat) {
   ...
   void allocate_NuclearDataGPU(NuclearData* NucData, NuclearData* NucData_gpu, int NucData_File_Index) {
       ...
       cudaMemcpy(NucData_gpu->TotalAtomNumber, NucData->TotalAtomNumber, sizeof(unsigned long int),
                  cudaMemcpyHostToDevice);
       cudaMemcpy(NucData_gpu->RotMat, NucData->RotMat, 9 * sizeof(float), cudaMemcpyHostToDevice);
       cudaMemcpy(NucData_gpu->NumberOfElements, NucData->NumberOfElements, *NucData->K * sizeof(unsigned int),
                  cudaMemcpyHostToDevice);
   ...
   void read_NuclearData(NuclearData* NucData, NucDataProperties* NucDataProp, InputFileData* InputData,
   ...
   void replication_read_NuclearData(NuclearData* NucData, NucDataProperties* NucDataProp, InputFileData* InputData,
                                     int NucData_File_Index) {
   ...
   void init_NuclearData(NuclearData* NucData, NuclearData* NucData_gpu, NucDataProperties* NucDataProp,
                         InputFileData* InputData, int NucData_File_Index) {
   ...
   void free_NuclearData(NuclearData* NucData, NuclearData* NucData_gpu) {
   ...
   void disp_NuclearData(NuclearData* NucData) {
       for (int k = 0; k < *NucData->TotalAtomNumber; k++) {
           cout << NucData->x[k] << " " << NucData->y[k] << " " << NucData->z[k] << " " << NucData->Nuc[k] << " "
                << "\n";
       }
   }
